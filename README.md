...
- `GenericConstraintsDemo` - Generics, variance e constraints para APIs reutilizaveis com seguranca de tipos
- `Linq` - Query com LINQ
- `LogicalOperatorsDemo` - Operadores lógicos
- `NullableReferenceTypesDemo` - Nullable annotations, análise de fluxo, operadores de null e guard clauses
...
- `SpanAndMemoryDemo` - Arrays, `Span<T>` e `Memory<T>` em slicing e parsing com menos alocacoes
...
#### 02-AsyncAndConcurrency (13 projetos)
- `Asynchronous` - Async/await básico
- `AsyncLockingDemo` - Race condition em seção crítica assíncrona e exclusão mútua com `SemaphoreSlim`
- `AsyncStreamsDemo` - `IAsyncEnumerable<T>`, `await foreach` e cancelamento com `[EnumeratorCancellation]`
...
- `HealthChecksApi` - Liveness e readiness, health checks de dependências e saída para orquestradores
...
- `ServerSentEventsDemo` - Streaming unidirecional com SSE, cancelamento e reconexão do cliente
...
#### 04-Authentication (11 projetos)
- `AdvancedAuthSystem` - Sistema avançado de autenticação
- `ApiKeyAuthenticationDemo` - Esquema de autenticação próprio para chaves de API, com escopos, rotação e revogação
- `Authentication` (2 subprojetos)
  - `Auth` - Autenticação básica
  - `OAuthApplication` - OAuth 2.0
- `PasskeyAuthenticationDemo` - Autenticação com passkeys (WebAuthn/FIDO2), resistente a phishing e sem senha
...
- `Caching` (11 subprojetos)
  ...
  - `DistributedCacheInvalidationDemo` - Invalidação de cache local entre instâncias com Redis Pub/Sub
...
- `ProxyRemoteServiceDemo` - Proxy virtual, de proteção e de telemetria sobre o mesmo contrato
...
#### 10-Algorithms (9 projetos)
- `BinarySearchBoundariesDemo` - Busca exata, lower bound e upper bound derivados do invariante `[lo, hi)`
- `DynamicProgrammingCoinChangeDemo` - Coin change com recursão, memoization e tabulation lado a lado
...
#### 11-Utilities (13 projetos)
- `ClassToDTO` - Conversão classe para DTO
- `ClassToXml` - Conversão classe para XML
- `CompressDecompress` - Compressão/descompressão
- `ConfigurationOptionsDemo` - Camadas de configuração, Options Pattern, validação no startup e recarga
...
- `FileChecksumDeduplicator` - Hash por stream, duplicatas por tamanho antes de conteúdo, colisões e buffers
